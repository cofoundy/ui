...
export function renderAgentsMarkdown(timestamp = new Date().toISOString()): string {
...
/**
 * Strip the timestamp comment line so byte-comparison ignores it.
 * The drift gate compares everything else.
 */
export function stripTimestamp(md: string): string {
  return md.replace(/<!-- Last generated: [^>]+-->\n/, '');
}
...
export function runCli(args: string[] = process.argv.slice(2), outputPath = OUTPUT_PATH): number {
  ...
  if (checkMode) {
    if (!existsSync(outputPath)) {
      console.error('[gen:agents] AGENTS.md does not exist. Run `pnpm gen:agents` and commit.');
      return 1;
    }
    const existing = readFileSync(outputPath, 'utf-8');
    if (stripTimestamp(existing) !== stripTimestamp(content)) {
      console.error('[gen:agents] DRIFT: AGENTS.md is out of date.');
      console.error('Run `pnpm gen:agents` and commit updated AGENTS.md.');
      return 1;
    }
